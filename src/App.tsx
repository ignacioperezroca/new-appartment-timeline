...
> = {
  current: {
    ...
    summary: "Close obligations, reduce clutter, repaint, and hand the place back without last-minute drag.",
    progress: 36,
    status: "Contract exit is active",
    steps: [
      { label: "Close current contract", range: "Day 1-12", progress: 58, dependency: "Notice + terms", icon: ClipboardCheck },
      { label: "Pack", range: "Day 8-30", progress: 46, dependency: "Room order", icon: PackageCheck },
      { label: "Paint", range: "Day 18-34", progress: 24, dependency: "Access + materials", icon: PaintRoller },
      { label: "Handoff / exit", range: "Day 35-45", progress: 12, dependency: "Clean evidence", icon: DoorOpen },
    ],
  },
  new: {
    eyebrow: "Get in",
    title: "New apartment",
    summary: "Secure access, move the core load, clean the space, and organize the first livable setup.",
    progress: 29,
    status: "Access runway forming",
    steps: [
      { label: "Sign new contract", range: "Day 1-10", progress: 64, dependency: "Docs + deposit", icon: KeyRound },
      { label: "Move", range: "Day 28-38", progress: 20, dependency: "Mover slot", icon: Truck },
      { label: "Clean", range: "Day 32-42", progress: 18, dependency: "Keys + supplies", icon: Sparkles },
      { label: "Organize / settle in", range: "Day 36-45", progress: 14, dependency: "First-night kit", icon: Home },
    ],
  },
};
  ...
  return (
    <div className="app-shell relative min-h-screen overflow-x-hidden text-[color:var(--text)] transition-colors duration-500">
      ...
      <main className="relative mx-auto flex w-full max-w-[1520px] flex-col gap-6 px-3 pb-16 pt-4 sm:px-5 lg:px-8 lg:pt-8">
        ...
        <MobileDayRail />
        <TwoPhaseJourney activeTab={activeTab} onSelectPhase={setActiveTab} />
        <MissionStrip />
...
function MobileDayRail() {
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, delay: 0.08 }}
      className="mobile-day-rail premium-surface px-4 py-3 md:hidden"
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="font-mono text-[10px] font-black uppercase tracking-[0.18em] text-[color:var(--flour)]">
            Day {currentTimelineDay} / {timelineTotalDays}
          </div>
          <div className="mt-1 text-sm font-black text-[color:var(--text-strong)]">
            Exit and arrival tracks
          </div>
        </div>
        <div className="text-right text-xl font-black text-[color:var(--flour)]">
          {overallProgress}%
        </div>
      </div>
      <ProgressBar value={overallProgress} className="mt-3 h-2" />
    </motion.div>
  );
}

function TwoPhaseJourney({
  activeTab,
  onSelectPhase,
}: {
  activeTab: TabId;
  onSelectPhase: (phase: TabId) => void;
}) {
  return (
    <AnimatedSection className="two-phase-shell premium-surface p-4 sm:p-5 lg:p-6">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div className="max-w-3xl">
          <div className="eyebrow text-[color:var(--flour)]">45-day master plan</div>
          <h2 className="mt-2 text-3xl font-black tracking-[-0.04em] text-[color:var(--text-strong)] sm:text-4xl">
            The move works when exit and arrival stay synchronized.
          </h2>
          <p className="mt-3 text-base leading-7 text-[color:var(--muted)]">
            The current apartment closes obligations while the new apartment unlocks livability. Day 5 is the active
            marker: every dependency either clears the exit lane or accelerates the arrival lane.
          </p>
        </div>

        <div className="rounded-[24px] border border-[color:var(--line)] bg-[color:var(--surface-raised)] p-4 shadow-sm">
          <div className="flex items-end justify-between gap-8">
            <div>
              <div className="font-mono text-[10px] font-black uppercase tracking-[0.2em] text-[color:var(--muted)]">
                Current day
              </div>
              <div className="mt-1 text-3xl font-black text-[color:var(--text-strong)]">
                {currentTimelineDay}
              </div>
            </div>
            <div className="text-right">
              ...
              <div className="mt-1 text-3xl font-black text-[color:var(--flour)]">
                {timelineTotalDays - currentTimelineDay}
              </div>
            </div>
          </div>
          <ProgressBar value={overallProgress} className="mt-4 h-3" />
        </div>
      </div>

      <div className="dual-track-grid mt-6">
        {(Object.keys(phaseTracks) as TabId[]).map((phase, phaseIndex) => (
          <PhaseTrackCard
            key={phase}
            phase={phase}
      ...
      <div className="handoff-bridge mt-5">
        <div className="handoff-node">Current apt closes</div>
        <div className="handoff-line">
          <span style={{ left: `${(currentTimelineDay / timelineTotalDays) * 100}%` }} />
        </div>
        <div className="handoff-node">New apt becomes livable</div>
      </div>
    </AnimatedSection>
  );
}
...
function PhaseTrackCard({
  ...
  return (
    <motion.button
      ...
      transition={{ duration: 0.5, delay: phaseIndex * 0.08, ease: [0.22, 1, 0.36, 1] }}
      className={cn(
        "phase-track-card text-left",
        active && "phase-track-card-active",
      )}
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="eyebrow text-[color:var(--flour)]">{track.eyebrow}</div>
          <h3 className="mt-2 text-2xl font-black tracking-[-0.04em] text-[color:var(--text-strong)]">
            {track.title}
          </h3>
          <p className="mt-2 max-w-xl text-sm leading-6 text-[color:var(--muted)]">{track.summary}</p>
        </div>
        <div className="rounded-[18px] border border-[color:var(--line)] bg-[color:var(--surface-soft)] px-4 py-3 text-right">
          <div className="text-2xl font-black text-[color:var(--text-strong)]">{track.progress}%</div>
          <div className="font-mono text-[10px] font-black uppercase tracking-[0.14em] text-[color:var(--muted)]">
            {track.status}
          </div>
        </div>
      </div>

      <div className="phase-step-lane mt-5">
        {track.steps.map((step, index) => {
          const Icon = step.icon;
          return (
            <div key={step.label} className="phase-step">
              <div className="flex items-center justify-between gap-2">
                <div className="grid h-9 w-9 place-items-center rounded-[16px] bg-[rgba(255,138,31,.14)] text-[color:var(--flour)]">
                  <Icon className="h-4 w-4" />
                </div>
                <span className="rounded-full border border-[color:var(--line)] bg-[color:var(--surface-soft)] px-2.5 py-1 font-mono text-[9px] font-black uppercase tracking-[0.12em] text-[color:var(--muted)]">
                  {step.range}
                </span>
              </div>
              <div className="mt-3 text-sm font-black text-[color:var(--text-strong)]">{step.label}</div>
              <div className="mt-1 min-h-5 text-xs font-semibold text-[color:var(--muted)]">{step.dependency}</div>
              <ProgressBar value={step.progress} className="mt-3 h-2" delay={index * 0.04} />
            </div>
          );
        })}
      </div>
